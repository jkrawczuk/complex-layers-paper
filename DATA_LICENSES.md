...
CSV export of the Alon et al. colon cancer microarray benchmark:
...
gene-expression features, and one class-label column, stored in a plain CSV format for Python reproducibility.
Feature standardization for the manuscript experiments is performed within each cross-validation split by the
experiment runner, not pre-applied to the CSV file.
...
repository when using the CSV.

For repository metadata and future archival releases, the license separation is:

- source code: MIT License;
- `data/synthetic_signal.csv` and its metadata: MIT License unless the release states otherwise;
- `data/colon_cancer.csv`: redistribution of the Bioconductor `colonCA` data under the upstream LGPL package
  license.
