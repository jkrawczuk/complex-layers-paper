...
decreasing strengths $s=(1.2,\;0.9,\;0.6)$, so source 1 is strongest and source 3 is weakest. The class score
is generated directly from these latent signals:
\[
...
generated CSV stores the raw feature values rather than globally standardized values. Because the benchmark
fixes the indices of the 12 informative features, it allows us to evaluate both predictive accuracy and the
number of ground-truth informative features recovered by each neuron and by the cumulative sequence of neurons.
...
cumulative recall to assess whether later neurons add new signal or mainly move toward noisy variables.
