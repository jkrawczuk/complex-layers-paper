...
pipeline: \texttt{cpl} (convex piecewise-linear; an LP-trained sparse neuron based on hinge loss with L1
regularization), \texttt{svm} (squared hinge loss with L1 regularization), and \texttt{logreg}. In each layer, one sparse neuron is trained and the
...
the sparsity regime. The accompanying repository is structured to support permanent archiving of the exact code
and data artifacts used in the study.
\end{abstract}
...
The source code, experiment scripts, input datasets, and final manuscript figure assets are available in the
project repository archived at [DOI to be added before submission].
The exact colon cancer file used in this study is \texttt{data/colon\_cancer.csv}; it corresponds to the
Alon et al. (1999) colon cancer microarray benchmark distributed by the Bioconductor \texttt{colonCA}
experiment data package (DOI: \texttt{10.18129/B9.bioc.colonCA}; LGPL).
